...
#ifdef __IOS__
    // Window dimensions saved by LUS are native drawable pixels. UIKit expects
    // logical points, so never feed those persisted Retina dimensions back
    // into SDL. Derive a fresh, landscape window from the current iOS display.
    SDL_Rect displayBounds = {};
    if (SDL_GetDisplayUsableBounds(0, &displayBounds) == 0 &&
        displayBounds.w > 0 && displayBounds.h > 0) {
        mWindowWidth = std::max(displayBounds.w, displayBounds.h);
        mWindowHeight = std::min(displayBounds.w, displayBounds.h);
    }
#endif
