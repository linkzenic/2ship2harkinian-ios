...
void Gui::DrawMenu() {
    ...
    if (anyMenuKeyPressed) {
        ...
        window->SetMouseCapture(!GetMenuOrMenubarVisible() && fastWindow->ShouldAutoCaptureMouse());
...
#else
#if defined(__IOS__) && !defined(__TVOS__)
    const bool touchMenuTogglePressed = TwoShipIOS_ConsumeMenuToggleRequest();
#else
    const bool touchMenuTogglePressed = false;
#endif
#if defined(__TVOS__)
...
#if defined(__IOS__) && !defined(__TVOS__)
    TwoShipIOS_SetTouchControlsMenuVisible(GetMenuOrMenubarVisible() ? 1 : 0);
#endif
#endif
