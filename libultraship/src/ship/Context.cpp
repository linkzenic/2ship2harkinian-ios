...
#if defined(__TVOS__)
    if (path == "2ship2harkinian.json") {
        const std::filesystem::path workingPath =
            std::filesystem::path(GetAppDirectoryPath(appName)) / path;
        std::error_code error;
        std::filesystem::create_directories(workingPath.parent_path(), error);
        TwoShipTVOS_PrepareSettingsJSON(workingPath.string().c_str());
        return workingPath.string();
    }
#endif
