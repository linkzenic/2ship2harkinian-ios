...
void Config::Reload() {
    ...
    const bool pathExists = mPath != "None" && fs::exists(mPath, pathError);
    const bool pathIsRegular = pathExists && !pathError && fs::is_regular_file(mPath, pathError);
    if (mPath == "None" || pathError || !pathExists || !pathIsRegular) {
        mIsNewInstance = true;
    ...
    try {
        ...
        std::ifstream backupStream(mPath + ".bak");
        try {
            mNestedJson = nlohmann::json::parse(backupStream);
            mFlattenedJson = mNestedJson.flatten();
        } catch (...) {
            mFlattenedJson = nlohmann::json::object();
        }
...
void Config::Save() {
    ...
    const std::string serialized = mNestedJson.dump(4);
    const std::string temporaryPath = mPath + ".tmp";
    const std::string backupPath = mPath + ".bak";
    ...
    const bool existingConfig = fs::exists(mPath, backupError);
    if (!backupError && existingConfig) {
        fs::copy_file(mPath, backupPath, fs::copy_options::overwrite_existing, backupError);
    }

    bool installed = std::rename(temporaryPath.c_str(), mPath.c_str()) == 0;
    if (!installed) {
        std::error_code replaceError;
        fs::remove(mPath, replaceError);
        installed = std::rename(temporaryPath.c_str(), mPath.c_str()) == 0;
        if (!installed) {
            fs::remove(temporaryPath, replaceError);
        }
    }
#if defined(__TVOS__)
    if (installed && mPath.ends_with("2ship2harkinian.json")) {
        TwoShipTVOS_PersistSettingsJSON(serialized.data(), serialized.size());
    }
#endif
}
