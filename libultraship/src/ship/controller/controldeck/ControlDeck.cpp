...
void ControlDeck::Init(uint8_t* controllerBits) {
    ...
    *mControllerBits |= 1 << 0;
    ...
    // Existing desktop/controller configuration files may not include the
    // virtual iOS gamepad. Always seed its standard mappings on Apple touch
    // devices so the file-select screen is operable on first launch.
    mPorts[0]->GetConnectedController()->AddDefaultMappings(PhysicalDeviceType::SDLGamepad);
#endif
}
