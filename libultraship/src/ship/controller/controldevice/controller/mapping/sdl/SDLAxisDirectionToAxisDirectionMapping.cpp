...
float SDLAxisDirectionToAxisDirectionMapping::GetNormalizedAxisDirectionValue() {
    ...
    if (mPortIndex == 0 &&
        ...
        float nativeX = 0.0f;
        float nativeY = 0.0f;
        TwoShipTVOS_GetNativeRightStick(&nativeX, &nativeY);
        // GameController Y is positive upward; SDL's standardized Y axes are positive downward.
        const float nativeSDLValue =
            mControllerAxis == SDL_CONTROLLER_AXIS_RIGHTX ? nativeX : -nativeY;
        if ((mAxisDirection == POSITIVE && nativeSDLValue >= 0.0f) ||
            (mAxisDirection == NEGATIVE && nativeSDLValue <= 0.0f)) {
            normalizedValues.push_back(fabsf(nativeSDLValue) * MAX_AXIS_RANGE);
        }
    }
#endif
