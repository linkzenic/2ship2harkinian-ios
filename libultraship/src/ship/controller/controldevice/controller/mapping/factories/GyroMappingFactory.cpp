...
#else
#if defined(__IOS__)
        if (!SDL_GameControllerHasSensor(gamepad, SDL_SENSOR_GYRO) && !appleControllerHasGyro) {
            continue;
        }
#else
...
#if defined(__IOS__)
    if (mapping == nullptr && appleControllerHasGyro &&
        (fabsf(appleGyroData[0]) > 0.15f || fabsf(appleGyroData[1]) > 0.15f ||
         fabsf(appleGyroData[2]) > 0.15f)) {
        mapping = std::make_shared<SDLGyroMapping>(portIndex, 1.0f, 0.0f, 0.0f, 0.0f);
        mapping->Recalibrate();
    }
#endif
