...
#if defined(__TVOS__)
    if (mapping == nullptr && portIndex == 0) {
        float nativeX = 0.0f;
        float nativeY = 0.0f;
        TwoShipTVOS_GetNativeRightStick(&nativeX, &nativeY);
        constexpr float nativeThreshold = 0.7f;
        if (fabsf(nativeX) >= nativeThreshold || fabsf(nativeY) >= nativeThreshold) {
            SDL_GameControllerAxis axis;
            int32_t axisDirection;
            if (fabsf(nativeX) >= fabsf(nativeY)) {
                axis = SDL_CONTROLLER_AXIS_RIGHTX;
                axisDirection = nativeX < 0.0f ? NEGATIVE : POSITIVE;
            } else {
                axis = SDL_CONTROLLER_AXIS_RIGHTY;
                // Convert Apple's positive-up Y axis to SDL's positive-down convention.
                axisDirection = nativeY > 0.0f ? NEGATIVE : POSITIVE;
            }
